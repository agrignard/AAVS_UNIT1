...
experiment cbd_digital_twins type: gui autorun:true{	
	...
	output {
		...
		{  
			...
			chart "Mode of Transport" type: pie style: ring background: rgb(242,242,242) color: rgb(236,102,45) label_text_color: rgb(236,102,45)  axes: #red  title_font: font( 'BrownPro', 32.0, #plain)
			tick_font: font('BrownPro' , 14, #plain) label_font: font('BrownPro', 32 #plain) x_label: 'Nice Xlabel' y_label:
			'Nice Ylabel' size:{0.5,0.5} position:{0,0.5}  label_background_color: rgb(242,242,242) tick_line_color: rgb(255,255,255)
			legend_font: font('BrownPro' , 14, #plain) 
			
			{
				data "Car" value: (1+cycle) color: rgb(71,42,22);
				data "Tram" value: (1+cycle) color: rgb(112,76,51);
				data "Bike" value: (1+cycle) color: rgb(161,106,69);
				data "Walk" value: (1+cycle) color: rgb(217,145,93);
				data "Bus" value: (1+cycle) color: rgb(237,179,140);
				data "Other" value:(1+cycle) color: rgb(244,169,160);
			...
			overlay position: { 50#px,50#px} size: { 1 #px, 1 #px } background: # black border: #black rounded: false
			{
				...
				draw "Building LandUse" at: {0, y} anchor: #top_left  color: #black font: font('BrownPro' , 14, #plain);
				y <- y + 50#px;
                loop z over: legends_pie.pairs
                {
                    draw square(40#px) at: { 20#px, y } color: rgb(z.key, 0.8) ;
                    draw z.value at: { 60#px, y} anchor: #left_center color: # white font: text;
                    y <- y + 40#px;
                }
			}
...
experiment cbd_digital_twins_demo type: gui autorun:true{	
	...
	output {
		display table_display type: 3d axes: false background: rgb(251, 227, 190) fullscreen:0{
			...
			overlay position: { 50#px,50#px} size: { 1 #px, 1 #px } background: # black border: #black rounded: false
			{
				draw "CBD ToolKIT v1.0" at: {0,0} color: #black font: font("Helvetica", 50, #bold);
